...
document.getElementById("save")!.addEventListener("click", async () => {
  const domains = domainsEl.value.split(",").map(s => s.trim()).filter(Boolean);
  await chrome.storage.sync.set({ domains, relayUrl: relayUrlEl.value, deviceToken: deviceEl.value });
  alert("Saved");
});

(async () => {
  const cfg = await chrome.storage.sync.get({ domains: [], relayUrl: "", deviceToken: "" });
