...
async function getSalt(): Promise<string> {
  if (salt) return salt;
  const { [SECRET_KEY_STORAGE]: v } =
    await chrome.storage.local.get(SECRET_KEY_STORAGE);
  if (v) {
    salt = v;
    return v;
  }
  const rand = crypto.getRandomValues(new Uint8Array(16));
  const s = Array.from(rand)
...
function isSensitiveKey(k: string): boolean {
  return /authorization|cookie|token|secret|password|apikey|session|bearer/i.test(
    k
  );
}
...
export const redact = {
  ...
  async token(v: string): Promise<string> {
    ...
    const digest = await subtleDigest(
      'SHA-256',
      new TextEncoder().encode(s + ':' + v)
    );
...
function redactJson(x: unknown): unknown {
  if (x == null) return x;
  if (typeof x === 'string') {
