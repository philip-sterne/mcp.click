...
window.addEventListener("click", (e) => {
  ...
  const label = target.innerText?.trim() || target.getAttribute("aria-label") || target.getAttribute("title") || "";
...
window.addEventListener("submit", (e) => {
  ...
  form.querySelectorAll("label[for]").forEach(l => labels.set(l.getAttribute("for")!, l.textContent?.trim() || ""));
  new FormData(form).forEach((v, k) => {
    const id = (form.querySelector(`[name="${CSS.escape(k)}"]`) as HTMLElement)?.id || "";
